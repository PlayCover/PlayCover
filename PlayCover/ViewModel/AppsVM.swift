    ...
    func fetchApps(){
        DispatchQueue.global(qos: .background).async {
            ...
            do {
                ...
                let directoryContents = try FileManager.default.contentsOfDirectory(at: PlayTools.playCoverContainer, includingPropertiesForKeys: nil, options: [])
                
                let subdirs = directoryContents.filter{ $0.hasDirectoryPath }
                
                for sub in subdirs {
                    if sub.pathExtension.contains("app") && fm.fileExists(atPath: sub.appendingPathComponent("Info.plist").path){
