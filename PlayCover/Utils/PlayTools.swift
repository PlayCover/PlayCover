...
class PlayTools {
    ...
    static func installInIPA(_ exec: URL, _ payload: URL, resign: Bool = false) throws {
        try Patch.patchBinaryWithDylib(binaryUrl: exec, dylibName: playToolsPath.path)
        if resign {
            shell.signApp(exec)
        }
    }
    ...
    static func injectInIPA(_ exec: URL, payload: URL) throws {
        try Patch.patchBinaryWithDylib(binaryUrl: exec, dylibName: "@executable_path/Frameworks/PlayTools.dylib")
        DispatchQueue.global(qos: .background).async {
            do {
