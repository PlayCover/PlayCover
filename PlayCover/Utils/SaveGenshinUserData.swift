...
func storeUserData(folderName: String, accountRegion: String) {
    ...
    let accountInfoPlistEncryptUrl = URL(fileURLWithPath: gameDataPath + accountInfoPlistEncrypt)
    let kibanaReportArrayKeyEncryptUrl = URL(fileURLWithPath: gameDataPath + kibanaReportArrayKeyEncrypt)
    let lastAccountModelEncryptUrl = URL(fileURLWithPath: gameDataPath + lastAccountModelEncrypt)
    ...
    do {
        ...
        try accountRegion.write(
            to: URL(fileURLWithPath: storePath + "region.txt"),
            atomically: false,
            encoding: .utf8)
...
func checkCurrentRegion(selectedRegion: String) throws -> Bool {
    // plist path
    let url = URL(
        fileURLWithPath: NSHomeDirectory() + "/Library/Containers/com.miHoYo.GenshinImpact/" +
    ...
    return value.contains(selectedRegion)
}
